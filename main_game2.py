...
# ВЕРСИЯ ПРОГРАММЫ
version = '0.9.1 '  # версия
# 0.8.1 -> 0.9.1 (05.01.2022)


# pip freeze > requirements.txt
# (делает файл requirements.txt с использованными библиотеками которых нет изначально в питоне)
# писать в консоль
...
# 5. Доделал секундомер
# 6. Сделал начальный экран
# 7. Сделал конечный экран (пока не работает, потом доработаю)
# 8. Враг теперь не может ходить сквозь коробки
# 9. Включил музыку
# 10. Добавил файл requirements.txt
...
def display_interface(k):
    if k % 3 == 0:
        ...
        draw_seconds(sc, 'time: ' + f'{(datetime.datetime.now() - start_time).seconds}', 30, 922, 2)
...
class StartWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        uic.loadUi('pyqt_button_pyagme.ui', self)
...
# ФИНАЛЬНОЕ ОКНО
class EndWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super(EndWindow, self).__init__()
        uic.loadUi('pygame_end_window.ui', self)
        self.pushButton.clicked.connect(self.start_button_pressed)
        self.show()
...
class Enemy(pygame.sprite.Sprite):
    ...
    def animation(self):  # анимация ходьбы
        if not pygame.sprite.spritecollide(self, obstacle_group, False):  # не столкнулся ли с ящиком
    ...
    def update(self):  # перемещение противника на карте
        if self.rect.x not in range(player.rect.x - 2, player.rect.x + 2):
            if pygame.sprite.spritecollide(self, obstacle_group, False):  # столкновение с ящиком: True
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if pygame.sprite.spritecollide(opponent, obstacle_group, False):  # оппонент когда сталкивается с ящиком
            opponent.rect.x += (opponent.x_last - opponent.x_map_enemy - 5)
            opponent.rect.y += (opponent.y_last - opponent.y_map_enemy - 5)
        ...
        opponent.x_last = opponent.x_map_enemy  # x персоонажа до столкновения
        opponent.y_last = opponent.y_map_enemy  # y персоонажа до столкновения
